                            ...
                            return (
                                ...
                                >
                                    <TableCell align="right">
                                        <Typography color="primary" component={"h6"}> {table.tableName}</Typography>
                                    </TableCell>
                                    {table.seats.map((seat, index) => (
                                        <TableCell align="right" key={index}>
                                            <Button variant="outlined" onClick={() => bookSeat(restaurant._id, table.tableName, seat.seat)}>
                                                {seat.status === "not-booked" ? 'Available' : 'Booked'}
                                            </Button>
                                        </TableCell>
                                    ))}
                                </TableRow>
                            );
                        }) || <TableRow><TableCell colSpan={5}>No tables available</TableCell></TableRow>}
                    </TableBody>
